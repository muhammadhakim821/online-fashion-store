...
const CartModal = ({products,isOpen,onClose})=>{
    return(
        <>
        <div className={`fixed z-[1000] inset-0 bg-black bg-opacity-50 transition-opacity ${isOpen ? "opacity-100" :
            "opacity-0 pointer-events-none"}`} 
            style={{transition:"opacity 300ms"}}>
                <div className={`fixed right-0 top-0 md:w-1/3 w-full overflow-y-auto bg-white h-full transition-transform ${isOpen ? 'translate-x-0':' -translate-x-full'}`}
                style={{transition:'transform 300ms cubic-bezier(0.25,0.46,0.45,0.94)'}}>
                    <div className="p-4 mt-4">
                        ...
                        <div className="cart-items">
                            {
                                products.length === 0 ? (<div>Your Cart is empaty</div>) : (
